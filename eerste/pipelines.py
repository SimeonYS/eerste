...
class EerstePipeline:

    # Database setup
    conn = sqlite3.connect('eerste.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `eerste`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM eerste WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO eerste (date, title, link, content)"
